...
function copy() {
	echo -e "Creating configuration directory under ${conf_dir}"
	mkdir -p ${conf_dir}/config

	echo -e "Copying configuration files from etc/cas to ${conf_dir}"
	cp -rfv etc/cas/* ${conf_dir}
}

function help() {
	echo "Usage: build.sh [copy|clean|package|run|debug|bootrun|gencert]"
	echo "	copy: Copy config from .${conf_dir}/config to ${conf_dir}/config"
...
function gencert() {
	...
	keytool -genkeypair -alias cas -keyalg RSA -keypass changeit -storepass changeit -keystore ${conf_dir}/thekeystore -dname ${DNAME} -ext SAN=${CERT_SUBJ_ALT_NAMES}
	keytool -exportcert -alias cas -storepass changeit -keystore ${conf_dir}/thekeystore -file ${conf_dir}/cas.cer
}
